...
proptest! {
    ...
    fn budget_deadline_is_min(d1 in arb_deadline(), d2 in arb_deadline()) {
        let b1 = match d1 {
            Some(t) => Budget::new().with_deadline(t),
            None => Budget::new(),
        };
        let b2 = match d2 {
            Some(t) => Budget::new().with_deadline(t),
...
proptest! {
    ...
    /// LAW: strengthen takes more severe
    ///
    /// After a.strengthen(b), a.kind == max(original_kind, b.kind) by severity.
    #[test]
    fn cancel_reason_strengthen_takes_max(k1 in arb_cancel_kind(), k2 in arb_cancel_kind()) {
        ...
        // The resulting kind should be whichever had higher severity
        if k1.severity() >= k2.severity() {
            prop_assert_eq!(a.kind, k1);
        } else {
            prop_assert_eq!(a.kind, k2);
        }
    }
}
